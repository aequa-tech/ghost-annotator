...
def aggregate_data(df_tipo_model, df_tipo_annotator, output_folder, modello, dataset,i):
    ...
    #
    #####
    label_probs = final_df['label'].value_counts(normalize=True).sort_index()

    # uniforme
    labels = label_probs.index
    uniform_probs = np.ones(len(labels)) / len(labels)
    ...
    output_file = os.path.join(output_folder, f"UNIFORMEstep_1_{modello}_{dataset}_{i}.csv")
