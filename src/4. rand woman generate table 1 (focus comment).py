...
def aggregate_data(df_tipo_model, df_tipo_annotator, output_folder, modello, dataset,i):
    ...
    #
    #####
    label_probs = final_df[final_df['social_group'] == 1]['label'].value_counts(normalize=True)
    print(label_probs)
    # WOMAN
    ...
    output_file = os.path.join(output_folder, f"WOMANstep_1_{modello}_{dataset}_{i}.csv")
