"""Compute ghost-annotator non conformity score (NCS) per comment.

Input:
- All CSV files in src_2/annotazione_modelli matching results_{MODEL}_{DATASET}.csv

Output:
- One CSV per input in src_2/ghost_annotatore/minimized/{MODEL}_{DATASET}.csv
- Columns: comment_id, ncs

NCS definition:
- Predicted label is the model argmax over probs (ghost annotator always picks model label).
- Non conformity is the mean Brier component over classes:
    mean_k ( (1 - p_k)^2 if k == y_hat else p_k^2 )
"""
...
OUTPUT_DIR = BASE_DIR / "ghost_annotatore" / "minimized"
...
def ghost_ncs(probs: dict[str, float]) -> float:
...
def process_one_file(input_path: Path) -> None:
    ...
    df["ncs"] = df["probs"].apply(lambda x: ghost_ncs(parse_probs(x)))
